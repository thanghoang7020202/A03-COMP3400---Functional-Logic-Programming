...
solve = do
    n <- readLn :: IO  Int
    let old = take n $ repeat 0
    print old
    (xs,ys) <- readLn :: IO ([Int], [Int])    
    if xs == [0..n-1] && ys == [] then return ()
    else do
        let newGuess = listGenerator n old 0 (xs,ys)
        print newGuess
        solver n newGuess

solver :: Int -> [Int] -> IO ()
solver n xs = do
    (ys,zs) <- readLn :: IO ([Int], [Int])
    if ys == [0..n-1] && zs == [] then return ()
    else do
        let newGuess = listGenerator n xs 0 (ys,zs)
        print newGuess
        solver n newGuess
-- receive and length of a list, old list, index and the tuple read from computer
listGenerator :: Int -> [Int] -> Int -> ([Int],[Int]) -> [Int]
listGenerator n xs k ([],_) = map (+1) xs
listGenerator n xs k (ys,_) = do
    if k < n then if elem k ys then (xs !! k): listGenerator n xs (k+1) (ys,[])
        else (xs !! k) + 1 : listGenerator n xs (k+1) (ys,[])
        else []

elementCMP:: [Int] -> [Int] -> Int -> Bool
elementCMP xs ys k = if ((length ys) <= k) then True
    else (xs !! (k)) /= (ys !! (k))
